...
function CreateCircle(unitID)
    ...
    texture:SetAtlas("groupfinder-icon-role-large-dps")
...
function UpdateCircle(texture, unit)
    ...
    if UnitExists(unit .. "target") then
        if targetNamePlate and texture then
            ...
            texture:SetPoint("CENTER", targetNamePlate, "CENTER", 0, 20)
            texture:Show()
...
local function eventHandler(self, event, unitID)
    ...
    elseif event == "NAME_PLATE_UNIT_REMOVED" then
        ...
        for unitID, texture in pairs(circleTextures) do
            local targetUnit = unitID .. "target"
            if UnitExists(targetUnit) and UnitGUID(targetUnit) == UnitGUID(unitID) then
                texture:Hide()
